...
export default function TaskForm({ task, onSubmitTask }) {
  ...
  const saveTask = (e) => {
    e.preventDefault();
    const tags = (newTask.tags instanceof Array) ? newTask.tags : newTask.tags?.split?.(/, */) || [];
    onSubmitTask({
      ...newTask,
      tags,
    });
  ...
  return (
    <form class="flex flex-col" onSubmit={saveTask}>
      ...
      <input className={INPUT} type="text" name="tags" value={newTask.tags} placeholder="Tags, comma separated" onChange={handleInputChange} />
      <button className="uppercase font-semibold tracking-wider outline-none bg-blue-400 border-2 border-blue-500 rounded p-1 text-white">Save</button>
    </form>
  )
}